...
Recent Changes:
- July 1, 2025: EDIT COMPANY MODAL RECONSTRUCTION COMPLETED - Completely rebuilt EditCompanyModal component to include all fields from AddCompanyModal with proper HTML tag stripping functionality. Added comprehensive form sections for basic info, description, categories, location, multimedia, social media, certificates and membership details. Implemented stripHtmlAndDecode function to clean textarea fields from HTML tags and entities. Fixed form validation and data pre-population for all 30+ company fields including arrays and complex objects.
- July 1, 2025: MEMBERSHIP LIMITS FRONTEND VALIDATION COMPLETED - Integrated MembershipLimitsValidator component into representative interfaces (RepresentativeCompanyManagementComplete and AddProjectModal) with real-time validation feedback. Added frontend enforcement preventing image upload when product limits exceeded and project creation when project limits reached. Enhanced user experience with clear error messages and system green color theming for CTA buttons.
- June 30, 2025: MEMBERSHIP LIMITS DISPLAY FIXED - Corrected frontend display of membership plan limits in representative dashboard to show "Ilimitado" when values are -1 instead of placeholder text. Fixed validation schema for unlimited options in membership form creation and editing. Added limits display section to public membership plan cards showing products, projects, and photos per project limits.
- June 30, 2025: PUBLIC PLANS ENHANCEMENT - Enhanced public membership plan cards to display detailed limits information (products, projects, photos per project) in a dedicated section before benefits, providing clear visibility of what each plan includes.
- June 30, 2025: MOST POPULAR PLAN SELECTION - Added administrative capability to mark any membership plan as "Most Popular" from dashboard with automatic badge display on public pages. Implemented exclusive selection logic ensuring only one plan can be marked as most popular at a time. Enhanced admin interface with clear visual indicators and improved public plan presentation with proper benefit formatting.
- June 30, 2025: BENEFITS FIELD EDITING FIXED - Corrected form data loading issue in membership plan editing where benefits field wasn't populating correctly. Fixed string/array conversion handling for benefits field in both create and edit workflows, ensuring proper data processing between form display and database storage.
- June 30, 2025: DIRECTORY CTA SECTION ADDED - Implemented call-to-action section below company map in directory view with professional blue gradient background, compelling messaging targeting equipment suppliers, and direct link to membership plans page for company registration.
...
- June 23, 2025: EMAIL CONFIGURATION SYSTEM COMPLETED - Implemented comprehensive email configuration system with SMTP settings management, persistent test email storage, professional email templates with company branding, robust error handling for common SMTP issues, and configuration testing with detailed error messages. Enhanced UI with provider-specific configuration tips and improved user feedback.
- June 23, 2025: STRIPE PERIODICIDAD SYNC FIXED - Corrected Stripe product synchronization to properly handle membership plan periodicities. Monthly plans now sync as monthly intervals, annual as yearly, quarterly as 3-month intervals, and semi-annual as 6-month intervals. Fixed database reference errors for stripeConfigurationTable and completed full Stripe integration testing.
- June 23, 2025: EDIT COMPANY MODAL COMPLETED - Implemented comprehensive company editing modal for administrators with complete field parity to creation form. Includes all 38 fields organized in sections: basic info, location, description, categories, certificates, tags, membership details. Fixed form validation, data pre-population, and file upload functionality.
