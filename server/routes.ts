...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/companies/register", async (req, res) => {
    try {
      ...
      if (!nombreEmpresa || !email1 || !paisesPresencia?.length || !ciudadesPresencia?.length) {
        return res.status(400).json({ 
          error: "Campos obligatorios faltantes",
          message: "Nombre de empresa, email, país y ciudad son obligatorios"
        });
      }

      // Preparar datos de la empresa para registro público
      const companyData = {
        nombreEmpresa,
        email1,
        telefono1: telefono1 || "",
        paisesPresencia: Array.isArray(paisesPresencia) ? paisesPresencia : [paisesPresencia],
        ciudadesPresencia: Array.isArray(ciudadesPresencia) ? ciudadesPresencia : [ciudadesPresencia],
        ubicacionGeografica: ubicacionGeografica || null,
        direccionFisica: direccionFisica || `${ciudadesPresencia[0]}, ${paisesPresencia[0]}`,
        representantesVentas: Array.isArray(representantesVentas) ? representantesVentas : [representantesVentas || ""],
        descripcionEmpresa: descripcionEmpresa || "Empresa registrada a través del formulario público",
        estado: "activo",
  ...
  // Public endpoint to get all active companies for map display
  app.get("/api/companies/public", async (req, res) => {
    try {
      ...
      // Filter only companies with geographic location for map display
      const companiesWithLocation = result.companies.filter(company => 
      ...
      // Return simplified company data for public consumption
      const publicCompanies = companiesWithLocation.map(company => ({
        id: company.id,
        nombreEmpresa: company.nombreEmpresa,
