...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ===================================
  // ENDPOINTS PARA MIGRACIÓN DE DATOS
  // ===================================
  ...
  app.get("/api/data/export", async (req, res) => {
    try {
      ...
      const membershipTypes = await storage.getAllMembershipTypes();
      const companiesResult = await storage.getAllCompanies({ includeInactive: true, limit: 1000 });
      ...
      const tags = await storage.getAllTags();
      const roles = await storage.getAllRoles();
      const opinionsResult = await storage.getAllOpinions({ limit: 1000 });
      const opinions = opinionsResult.opinions;
      const projectsResult = await storage.getAllProjects({ limit: 1000 });
      const projects = projectsResult.projects;

      // Para los pagos de membresía, usar un método simple si existe o crear uno básico
      let membershipPayments: any[] = [];
      try {
        // Intentar obtener todos los pagos - esto podría necesitar implementación
        membershipPayments = [];
      } catch (error) {
        console.log("No se pudieron obtener los pagos de membresía");
      }

      // Configuraciones del sistema
      const integrationSettings = await storage.getIntegrationSettings();
      const pdfSettings = await storage.getPdfSettings();
      const frontendConfiguration = await storage.getFrontendConfiguration();
      ...
      const exportData = {
        ...
        metadata: {
          ...
          totalMembershipTypes: membershipTypes.length,
        }
      };

      console.log(`✅ Exportación completada: ${companies.length} empresas, ${users.length} usuarios, ${categories.length} categorías`);
      
      res.setHeader('Content-Type', 'application/json');
      res.setHeader('Content-Disposition', 'attachment; filename="anpr-data-export.json"');
      res.json(exportData);

    } catch (error: any) {
      console.error("❌ Error en exportación:", error);
      res.status(500).json({ error: error.message || "Error exportando datos" });
    }
  });

  // Importar datos a la base de datos
  app.post("/api/data/import", express.json({ limit: '50mb' }), async (req, res) => {
    try {
      ...
      // 1. Importar roles primero (base para usuarios)
      if (importData.roles && Array.isArray(importData.roles)) {
        try {
          ...
          importResults.errors.push(`Roles: ${error.message}`);
        }
      }

      // 2. Importar membership types (base para empresas)
      ...
      if (importData.categories && Array.isArray(importData.categories)) {
        try {
          ...
          importResults.errors.push(`Categories: ${error.message}`);
        }
      }

      // 4. Importar certificados
      if (importData.certificates && Array.isArray(importData.certificates)) {
        try {
          for (const certificate of importData.certificates) {
            await storage.createCertificate(certificate);
            importResults.certificates++;
          }
        } catch (error: any) {
          importResults.errors.push(`Certificates: ${error.message}`);
        }
      }
      ...
      if (importData.tags && Array.isArray(importData.tags)) {
        try {
          ...
          importResults.errors.push(`Tags: ${error.message}`);
        }
      }
      ...
      if (importData.projects && Array.isArray(importData.projects)) {
        try {
          ...
          importResults.errors.push(`Projects: ${error.message}`);
        }
      }

      // 9. Importar opiniones
      if (importData.opinions && Array.isArray(importData.opinions)) {
        try {
          for (const opinion of importData.opinions) {
            await storage.createOpinion(opinion);
            importResults.opinions++;
          }
        } catch (error: any) {
          importResults.errors.push(`Opinions: ${error.message}`);
        }
      }

      // 10. Importar pagos de membresía
      if (importData.membershipPayments && Array.isArray(importData.membershipPayments)) {
        try {
          for (const payment of importData.membershipPayments) {
            await storage.createMembershipPayment(payment);
            importResults.membershipPayments++;
          }
        } catch (error: any) {
          importResults.errors.push(`Membership Payments: ${error.message}`);
        }
      }

      // 11. Importar configuraciones del sistema (simplificado)
      try {
        if (importData.integrationSettings) {
          // Para configuraciones, intentar crear o actualizar según exista
          const existing = await storage.getIntegrationSettings();
          if (existing) {
            await storage.updateIntegrationSettings(existing.id, importData.integrationSettings);
          } else {
            await storage.createIntegrationSettings(importData.integrationSettings);
          }
        }
        if (importData.frontendConfiguration) {
          const existing = await storage.getFrontendConfiguration();
          if (existing) {
            await storage.updateFrontendConfiguration(existing.id, importData.frontendConfiguration);
          } else {
            await storage.createFrontendConfiguration(importData.frontendConfiguration);
          }
        }
      } catch (error: any) {
        importResults.errors.push(`System configurations: ${error.message}`);
      }
      ...
      res.status(500).json({ error: error.message || "Error importando datos" });
    }
  });
  ...
  app.get("/api/data/status", async (req, res) => {
    try {
      ...
      const status = {
        ...
        users: users.length,
        isEmpty: companies.length === 0 && users.length === 0,
      ...
      res.status(500).json({ error: error.message || "Error verificando estado" });
    }
  });
